...
export function useResizeObserver<T extends HTMLElement = HTMLElement>(
	...
	const onResize = React.useRef<ResizeHandler | undefined>(undefined);
	...
	React.useEffect(() => {
		...
		const observer: ResizeObserver = new ResizeObserver(([entry]) => {
			...
			const newWidth: number = extractSize(entry, boxProp, 'inlineSize');
			const newHeight: number = extractSize(entry, boxProp, 'blockSize');
...
function extractSize(entry: ResizeObserverEntry, box: BoxSizesKey, sizeType: keyof ResizeObserverSize): number {
	if (!entry[box]) {
